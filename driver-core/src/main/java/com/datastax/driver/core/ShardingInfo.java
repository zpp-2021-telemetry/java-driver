...
public class ShardingInfo {
    ...
    private ShardingInfo(int shardsCount, String partitioner, String shardingAlgorithm, int shardingIgnoreMSB) {
    ...
    public static ConnectionShardingInfo parseShardingInfo(Map<String, List<String>> params) {
        ...
        if (shardId == null || shardsCount == null || partitioner == null || shardingAlgorithm == null || shardingIgnoreMSB == null
                || !partitioner.equals("org.apache.cassandra.dht.Murmur3Partitioner")
                || !shardingAlgorithm.equals("biased-token-round-robin")) {
            return null;
        }
        return new ConnectionShardingInfo(shardId, new ShardingInfo(shardsCount, partitioner, shardingAlgorithm, shardingIgnoreMSB));
    }
