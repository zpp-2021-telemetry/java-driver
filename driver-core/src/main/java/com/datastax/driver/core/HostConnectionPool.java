...
class HostConnectionPool implements Connection.Owner {
    ...
    ListenableFuture<Void> initAsyncWithConnection(Connection reusedConnection) {
        ...
        final int shardsCount = host.getShardingInfo() == null ? 1 : host.getShardingInfo().getShardsCount();
        ...
        waiter = new AtomicInteger[shardsCount];
        waitLock = new Lock[shardsCount];
        hasAvailableConnection = new Condition[shardsCount];
        ...
        for (int i = 0; i < shardsCount; ++i) {
            ...
            waiter[i] = new AtomicInteger();
            waitLock[i] = new ReentrantLock(true);
            hasAvailableConnection[i] = waitLock[i].newCondition();
        }
    ...
    private void addCallback(final List<Connection> connections, final List<ListenableFuture<Void>> connectionFutures, final SettableFuture<Void> initFuture,
            final List<Connection> toClose) {
        final Executor initExecutor = manager.cluster.manager.configuration.getPoolingOptions().getInitializationExecutor();
            ...
            public void onSuccess(List<Void> l) {
                ...
                if (isClosed()) {
                    ...
                    for (final List<Connection> shardsConnections : HostConnectionPool.this.connections) {
                        needed += Math.max(0, HostConnectionPool.this.connectionsPerShard - shardsConnections.size());
                    }
                    if (needed > 0) {
                        final List<Connection> connections = manager.connectionFactory().newConnections(HostConnectionPool.this, needed);
                        final List<ListenableFuture<Void>> connectionFutures = Lists.newArrayListWithCapacity(needed);
    ...
    public Connection borrowConnection(long timeout, TimeUnit unit, ByteBuffer routingKey) throws ConnectionException, TimeoutException {
        ...
        if (connections[shardId].isEmpty()) {
            if (!host.convictionPolicy.canReconnectNow())
                ...
                Connection c = waitForConnection(timeout, unit, shardId);
        ...
        if (leastBusy == null) {
            ...
            leastBusy = waitForConnection(timeout, unit, shardId);
        } else {
            while (true) {
    ...
    private void awaitAvailableConnection(long timeout, TimeUnit unit, int shardId) throws InterruptedException {
        waitLock[shardId].lock();
        waiter[shardId].incrementAndGet();
        try {
            hasAvailableConnection[shardId].await(timeout, unit);
        } finally {
            waiter[shardId].decrementAndGet();
            waitLock[shardId].unlock();
        }
    }

    private void signalAvailableConnection(int shardId) {
        // Quick check if it's worth signaling to avoid locking
        if (waiter[shardId].get() == 0)
            return;

        waitLock[shardId].lock();
        try {
            hasAvailableConnection[shardId].signal();
        } finally {
            waitLock[shardId].unlock();
        }
    }
    ...
    private void signalAllAvailableConnection() {
        for (int shardId = 0; shardId < waiter.length; ++shardId) {
            // Quick check if it's worth signaling to avoid locking
            if (waiter[shardId].get() == 0)
                continue;
            ...
            waitLock[shardId].lock();
            try {
                hasAvailableConnection[shardId].signalAll();
            } finally {
                waitLock[shardId].unlock();
            }
        }
    ...
    private Connection waitForConnection(long timeout, TimeUnit unit, int shardId) throws ConnectionException, TimeoutException {
        if (timeout == 0)
        ...
        do {
            try {
                awaitAvailableConnection(remaining, unit, shardId);
    ...
    public void returnConnection(Connection connection) {
        ...
        if (connection.state.get() != TRASHED) {
            if (connection.maxAvailableStreams() < minAllowedStreams) {
                ...
                signalAvailableConnection(connection.shardId());
            }
        }
    }
    ...
    private boolean addConnectionIfUnderMaximum(int shardId) {
        ...
        try {
            ...
            signalAvailableConnection(newConnection.shardId());
            return true;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private void maybeSpawnNewConnection(int shardId) {
        ...
        while (true) {
            ...
            if (inCreation >= MAX_SIMULTANEOUS_CREATION)
                return;
            if (scheduledForCreation[shardId].compareAndSet(inCreation, inCreation + 1))
                break;
        }
