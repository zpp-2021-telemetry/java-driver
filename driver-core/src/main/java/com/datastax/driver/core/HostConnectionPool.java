...
class HostConnectionPool implements Connection.Owner {
    ...
    private Queue<SettableFuture<Connection>> pendingBorrows[];
    ...
    ListenableFuture<Void> initAsyncWithConnection(Connection reusedConnection) {
        ...
        for (int i = 0; i < shardsCount; ++i) {
            ...
            pendingBorrows[i] = new ConcurrentLinkedQueue<SettableFuture<Connection>>();
        }
    ...
    ListenableFuture<Connection> borrowConnection(int maxQueueSize, ByteBuffer routingKey) {
        ...
        if (connections[shardId].isEmpty()) {
            if (host.convictionPolicy.canReconnectNow()) {
                ...
                return enqueue(maxQueueSize, shardId);
            }
        }
    ...
    private ListenableFuture<Connection> enqueue(int maxQueueSize, int shardId) {
        if (maxQueueSize == 0) {
        ...
        pendingBorrows[shardId].add(future);
    ...
    private void dequeue(Connection connection) {
        while (!pendingBorrows[connection.shardId()].isEmpty()) {
            ...
            final SettableFuture<Connection> pendingBorrow = pendingBorrows[connection.shardId()].poll();
    ...
    final CloseFuture closeAsync() {
        ...
        for (Queue<SettableFuture<Connection>> queue : pendingBorrows) {
            for (SettableFuture<Connection> pendingBorrow : queue) {
